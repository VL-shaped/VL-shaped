...
int main ( int argc, char** argv )
{
  ...
  while( fgets(buffer, 15, pFile ) )
  {
      ...
      unsigned int registers = instruction[1]; // byte0=opcode, byte1=two registers
      unsigned int regA = instruction[1]>>4; //register 1
      unsigned int regB = instruction[1]&0xf; //register 2
      unsigned int immValue = instruction[2] ^ instruction[3]<<8 ^ instruction[4]<<16 ^ instruction[5]<<24; //an immediate value, byte2->byte5 = 32bit integer
      unsigned int memAddress = instruction[1] ^ instruction[2]<<8 ^ instruction[3]<<16 ^ instruction[4]<<24; //a memory address, byte1->byte4 = 32bit integer
